                        ...
                        <div class="card-body">
                            <div class="row gutters">
                                ...
                                <form enctype="multipart/form-data" action="{{ route('user.update', Auth::user()->id) }}"
                                    method="POST" class="col-xl-12 col-lg-12 col-md-6 col-sm-6 col-12">
                                    @csrf
                                    @method('PUT')
                ...
                <tbody>
                    @foreach ($user as $item)
                        <tr style="text-align: center">
                            ...
                            <td>{{ $item->phone }}</td>
                            <td><a href="{{ route('company.show', $item->company_id) }}">{{ $item->company_name }}</a>
                            </td>
                            <td>{{ $item->price }} JD</td>
                            <td>{{ $item->date }}</td>
                            <td>
                                <span
                                    class="text-white p-1 @if ($item->booking_status == 'completed') bg-success @endif @if ($item->booking_status == 'pending') bg-warning @endif">{{ $item->booking_status }}</span>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
